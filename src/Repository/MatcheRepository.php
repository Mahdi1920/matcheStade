...
class MatcheRepository extends ServiceEntityRepository
{
    ...
    public function getOrderbynbSpectateurs()
    {
        $qb = $this->createQueryBuilder('m')
            ->orderBy('m.nbSpectateurs', 'Asc');
        return $qb->getQuery()->getResult();
    }

    public function searchMatch($numeroMatche)
    {
        $qb = $this->createQueryBuilder('m')
            ->where('m.numeroMatche = :d')
            ->setParameter('d', $numeroMatche);
        return $qb->getQuery()->getResult();
    }

    public function getMatchesByStade($id)
    {
        $s = $this->createQueryBuilder('m')
            ->join('m.stade', 's')
            ->addSelect('s')
            ->where('s.id=:id')
            ->groupBy('s.nom')
